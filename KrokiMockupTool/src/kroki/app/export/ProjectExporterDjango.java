 ...
 *
 */
public class ProjectExporterDjango {
	 ...
	 * @param message
	 */
	public void export(String exportApsolutePath, BussinesSubsystem proj, String message) {
		this.project = proj;
		this.exportApsolutePath=exportApsolutePath;
	...
	public void getStandardPanelData(VisibleElement el, String classPackage, Menu menu) throws NoZoomPanelException {
		...
		//if(!swing) {
			//pack = "adapt.entities";
		//}
		EJBClass ejb = new EJBClass("", sys, sp.getPersistentClass().name(), tableName, sp.getLabel(), attributes);
		classes.add(ejb);

		//       SUBMENU GENERATION DATA
		//each panel get it's own menu item
		String activate = ejb.getName().toLowerCase() + "_st";  // OVO PRILAGODI
	...
	public EJBAttribute getZoomData(Zoom z, String className) {
		if(z.getTargetPanel() != null) {
			...
			EJBAttribute attribute = new EJBAttribute(null, type, propName, label, databaseName, mandatory, false, false, null);
			return attribute;
